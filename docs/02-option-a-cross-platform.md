...
│   ├── settings.{h,cpp}     # ini-compatible settings (preserve key names,
│   │                        #   including LReSycn/RReSycn typos)
...
└── tests via ctest          # 10 headless tests (cli/*-test.cpp)
```
...
3. CLI tool: WAV (22050 Hz 16-bit mono) → `.syn` + PNG.
