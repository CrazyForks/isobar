...
- Units:
  ...
  - Form9: input device select (入力デバイスの選択; enumerates via
    `waveInGetNumDevs`/`waveInGetDevCapsA`; the mixer button prefers
    `SNDVOL32.EXE /RECORD` when `FindExecutable` succeeds, falling back
    to `rundll32 shell32.dll,Control_RunDLL mmsys.cpl @1`).
...
3. **Quadrature FM demod (Hilbert + atan2)**:
   ...
   - Reference subtract: `−1.425 + 0.996 + 0.00156` = **−0.42744 rad
     exactly** — ≈ −2π·1500/22050 (−0.42743) but deliberately not equal
     to it; referenced to the 1500 Hz black carrier.
...
8. **Fallback sync tracking** (the path that does the real work). Runs only
   ...
   **asymmetry**, `SyncWidth` to the left and the `syn` combo window to the
   ...
   Hysteresis, from the counters at `dword_4ED884`/`dword_4ED888`. Both
   keys are **release counters**, one per tracking mode:
   - **`LReSycn`** — consecutive *invalid* lines before a fine lock is
     dropped.
   - **`RReSycn`** — the same release count while the tracker is in its
     coarse mode; it doubles as the promotion count — that many
     consecutive *valid* lines promote coarse → fine (declares lock,
     sets the ever-locked flag at `+1012`).

   These two are easy to swap; both release, and `RReSycn` is also the
   one that acquires.
...
- 4096-point radix-2 DIT FFT (twiddles `dbl_51EAE8`/`dbl_526AE8` =
  ...
  the 2π/4096 step itself constructed at runtime from two immediates
  rather than stored as a static constant).
- Input: BPF output × Hann window (`dbl_54AAE8[i] = 0.5 −
  0.5·cos(2πi/4096)`), applied to only the first ~1102 samples of the
  block; the rest of the 4096-point input is zeroed.
...
| `[Sync]` | `LReSycn` | 10 | release counter, fine lock: invalid lines → drop lock (+1016; typo "Sycn" preserved) |
| `[Sync]` | `RReSycn` | 5 | release counter, coarse mode; doubles as valid lines → promote to lock (+1024) |
...
`count = byte0*255 + byte1`). **The width is not stored anywhere in the
file** — the line count is the only dimension in the header; the width
is fixed at 1500 px by the program itself. On load the original fills
lines N…2279 with black and groups the column view as count/3 +
count%3. The legacy `"Syn Fax"` variant is a different body layout, not
just a different magic: NO count field, and after mode+negative the
body is a fixed 2000 bytes/line × 2280 lines, decimated 4→3 into
1500-px lines by `out[(i*3)>>2] = in[i]` (input-driven, keeps 3 of
every 4 samples).
...
- **Demod constant puzzle**: `−1.425 + 0.996 + 0.00156` nets exactly
  −0.42744 — ≈ −2π·1500/22050 (−0.42743) but not equal; the split
  suggests hand-tuned "demodulation characteristic" adjustments.
  Preserve the sum.
...
| Demod reference & scale | −0.42744 rad (≈ 2π·1500/22050, not equal), ×1102.25 |
...
| Sync validity window | period 3981–4019 samples, pulse width 101–399 @ 8 kHz (both strict: reject ≤3980/≥4020, ≤100/≥400) |
